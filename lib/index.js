...
leave \`${MANIFEST_PATH}\` itself untouched:
    ...
    jq 'del(.components[].sources)' ${MANIFEST_PATH} > /tmp/arch-render.json
    node bin/archify.mjs deliver architecture /tmp/arch-render.json ${RENDER_PATH} --quality showcase --json`
...
/**
 * Strip the full-screen chrome from a rendered page so it reads as a panel.
 *
 * Archify already ships this: `html[data-embed="true"]` hides the toolbar, header, cards,
 * nav, overview map, focus chip and lens, and unpins the container width. Its own
 * bootstrap sets that attribute from `?embed=1`, but a `srcDoc` document has no query
 * string — so set it on the element directly. An attribute rather than an injected
 * script, so it applies before first paint and needs nothing to execute.
 */
export const embedify = (html) => html.replace('<html ', '<html data-embed="true" ')
...
export function createManifestHandler (ctx) {
  return async (endpoint, payload, signal) => {
    ...
    if (!root) return { ok: true, value: { root: null, manifest: null, render: null, stale: false } }
    ...
    try {
      ...
      return {
        ...
        value: {
          root,
          manifest,
          render: render === undefined ? null : embedify(render),
