...
/**
 * `str_replace_editor` is not a mutation tool — it is a file tool with four commands,
 * and `view` is a read (dsh-tool-str-replace-editor: "The schema provides `view`,
 * `create`, `str_replace`, and `insert`"). Filtering on the tool name alone marks every
 * file the agent merely looked at through it as edited.
 *
 * Tools with no `command` argument at all are unconditional mutations.
 */
const MUTATING_COMMANDS = new Set(['create', 'str_replace', 'insert'])
...
const mutates = (args) => args?.command === undefined || MUTATING_COMMANDS.has(args.command)
